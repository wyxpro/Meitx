...
| Endpoint | `GET https://app-crzi248am2gx-api-GaDwZ0j3erOY-gateway.appmiaoda.com/geocoding/v3/` |
| 认证模式 | `platform_managed`（`traefik: true`） |
| Auth Header | `X-Gateway-Authorization: Bearer <INTEGRATIONS_API_KEY>` |
| third_part_domain | `app-crzi248am2gx-api-GaDwZ0j3erOY-gateway.appmiaoda.com` |
| 计费 | 免费，但会统计调用次数 |
...
async function geocodeAddress(
  ...
  const response = await fetch(
    `https://app-crzi248am2gx-api-GaDwZ0j3erOY-gateway.appmiaoda.com/geocoding/v3/?${params.toString()}`,
    {
      method: "GET",
      headers: {
