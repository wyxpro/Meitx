...
| Endpoint | `GET https://app-crzi248am2gx-api-baBwZEjbe1X9-gateway.appmiaoda.com/reverse_geocoding/v3` |
| 认证模式 | `platform_managed`（`traefik: true`） |
| Auth Header | `X-Gateway-Authorization: Bearer <INTEGRATIONS_API_KEY>` |
| Content-Type | `application/x-www-form-urlencoded`（GET 请求参数附在 URL 上） |
| third_part_domain | `app-crzi248am2gx-api-baBwZEjbe1X9-gateway.appmiaoda.com` |
...
async function reverseGeocode(
  ...
  const response = await fetch(
    `https://app-crzi248am2gx-api-baBwZEjbe1X9-gateway.appmiaoda.com/reverse_geocoding/v3?${params.toString()}`,
    {
      method: "GET",
      headers: {
