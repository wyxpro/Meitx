...
| 接口 | 方法 | Endpoint | 核心参数 |
|------|------|----------|---------|
| 道路实时路况查询 | GET | `https://app-crzi248am2gx-api-rLobR3D3dbg9-gateway.appmiaoda.com/traffic/v1/road` | `road_name`（必填）、`city`（可选） |
| 矩形区域实时路况查询 | GET | `https://app-crzi248am2gx-api-ra5ErGpGM8wa-gateway.appmiaoda.com/traffic/v1/bound` | `bounds`（必填，左下角;右上角坐标） |
| 多边形区域实时路况查询 | GET | `https://app-crzi248am2gx-api-BYdwQ5e51blL-gateway.appmiaoda.com/traffic/v1/polygon` | `polygon`（必填，多边形坐标点序列） |
| 周边实时路况查询 | GET | `https://app-crzi248am2gx-api-Xa6JeEnEb2na-gateway.appmiaoda.com/traffic/v1/around` | `center`（必填，中心坐标）、`radius`（必填，单位米） |
...
const response = await fetch(
  "https://app-crzi248am2gx-api-rLobR3D3dbg9-gateway.appmiaoda.com/traffic/v1/road" +
