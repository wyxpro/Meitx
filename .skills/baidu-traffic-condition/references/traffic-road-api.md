...
| Endpoint | `GET https://app-crzi248am2gx-api-rLobR3D3dbg9-gateway.appmiaoda.com/traffic/v1/road` |
| Auth | `platform_managed`（`X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}`） |
| Content-Type | `application/json`（GET 请求，参数通过 Query String 传递） |
| third_part_domain | `app-crzi248am2gx-api-rLobR3D3dbg9-gateway.appmiaoda.com` |
...
async function queryRoadTraffic(
  ...
  const response = await fetch(
    `https://app-crzi248am2gx-api-rLobR3D3dbg9-gateway.appmiaoda.com/traffic/v1/road?${params.toString()}`,
    {
      method: "GET",
      headers: {
