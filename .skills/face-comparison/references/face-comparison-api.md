...
| Endpoint | `POST https://app-crzi248am2gx-api-5YrZz81oerkY-gateway.appmiaoda.com/rest/2.0/face/v3/match` |
| Auth 模式 | `platform_managed`（密钥由平台注入） |
| Auth Header | `X-Gateway-Authorization: Bearer ${apiKey}` |
| Content-Type | `application/json` |
| third_part_domain | `app-crzi248am2gx-api-5YrZz81oerkY-gateway.appmiaoda.com` |
...
async function callFaceMatch(face1: FaceImage, face2: FaceImage): Promise<FaceMatchResult> {
  const response = await fetch("https://app-crzi248am2gx-api-5YrZz81oerkY-gateway.appmiaoda.com/rest/2.0/face/v3/match", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
serve(async (req: Request): Promise<Response> => {
  ...
  const upstream = await fetch(
    "https://app-crzi248am2gx-api-5YrZz81oerkY-gateway.appmiaoda.com/rest/2.0/face/v3/match",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
