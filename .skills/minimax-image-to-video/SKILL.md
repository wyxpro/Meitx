...
async function pollVideoStatus(taskId: string): Promise<{
  ...
  const response = await fetch(
    `https://app-crzi248am2gx-api-eLMlPNkelVj9-gateway.appmiaoda.com/v1/query/video_generation?task_id=${encodeURIComponent(taskId)}`,
    {
      method: "GET",
      headers: {
        "X-Gateway-Authorization": `Bearer ${apiKey}`,
...
async function retrieveVideoFile(fileId: string): Promise<string> {
  const response = await fetch(
    `https://app-crzi248am2gx-api-rLyOyznAK2Ba-gateway.appmiaoda.com/v1/files/retrieve?file_id=${fileId}`,
    {
      method: "GET",
      headers: {
        "X-Gateway-Authorization": `Bearer ${apiKey}`,
  ...
  while (Date.now() < deadline) {
    ...
    if (queryJson.status === "Success") {
      ...
      const retrieveResponse = await fetch(
        `https://app-crzi248am2gx-api-rLyOyznAK2Ba-gateway.appmiaoda.com/v1/files/retrieve?file_id=${queryJson.file_id}`,
        {
          method: "GET",
          headers: { "X-Gateway-Authorization": `Bearer ${apiKey}` },
        }
