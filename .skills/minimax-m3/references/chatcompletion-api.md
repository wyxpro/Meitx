...
| 主调用接口 | `POST https://app-crzi248am2gx-api-rLobPAn0n7m9-gateway.appmiaoda.com/v1/chat/completions` |
| Endpoint 形式 | `https://{APP_ID}@app-crzi248am2gx-api-rLobPAn0n7m9-gateway.appmiaoda.com/v1/chat/completions` |
| 官方来源接口 | `POST https://app-crzi248am2gx-api-rLobPAn0n7m9-gateway.appmiaoda.com/v1/chat/completions`（仅用于接口来源说明，Skill 调用使用 `APP_ID@` 网关形式） |
| Third-party Domain | `app-crzi248am2gx-api-rLobPAn0n7m9-gateway.appmiaoda.com` |
...
MiniMax-M3 支持 1M 超长上下文、Coding/Agentic 场景、多模态理解和可控深度思考。当前 Skill 使用平台托管网关调用 MiniMax Chat Completions API，Endpoint 必须采用 `APP_ID@app-crzi248am2gx-api-rLobPAn0n7m9-gateway.appmiaoda.com` 形式，密钥由平台注入，文档中不要写入真实 API Key。
...
async function callMiniMaxChat(request: ChatCompletionRequest): Promise<ChatCompletionResponse> {
  const response = await fetch(
    "https://app-crzi248am2gx-api-rLobPAn0n7m9-gateway.appmiaoda.com/v1/chat/completions",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
