...
async function submitVideoGeneration(
  ...
  const response = await fetch(
    "https://app-crzi248am2gx-api-oLpZbv47qvea-gateway.appmiaoda.com/v1/video_generation",
    {
      method: "POST",
      headers: {
...
async function queryVideoGeneration(
  ...
  const response = await fetch(
    `https://app-crzi248am2gx-api-GYX1bq2l5vWa-gateway.appmiaoda.com/v1/query/video_generation?task_id=${encodeURIComponent(taskId)}`,
    {
      method: "GET",
      headers: {
        "X-Gateway-Authorization": `Bearer ${apiKey}`,
...
async function retrieveVideoFile(fileId: string): Promise<{ downloadUrl: string; filename: string }> {
  const response = await fetch(
    `https://app-crzi248am2gx-api-VaOw5V2Pbqoa-gateway.appmiaoda.com/v1/files/retrieve?file_id=${encodeURIComponent(fileId)}`,
    {
      method: "GET",
      headers: {
        "X-Gateway-Authorization": `Bearer ${apiKey}`,
