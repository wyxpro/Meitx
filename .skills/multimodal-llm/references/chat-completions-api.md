...
| Endpoint | `POST https://app-crzi248am2gx-api-k93RZBjPykEa-gateway.appmiaoda.com/v2/chat/completions` |
| 认证模式 | `platform_managed` |
| Auth Header | `X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}` |
| Content-Type | `application/json` |
| 响应类型 | SSE 流式（`text/event-stream`） |
| third_part_domain | `app-crzi248am2gx-api-k93RZBjPykEa-gateway.appmiaoda.com` |
...
async function callMultimodalChat(
  ...
  const response = await fetch(
    "https://app-crzi248am2gx-api-k93RZBjPykEa-gateway.appmiaoda.com/v2/chat/completions",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
...
- 前端只与 Supabase Edge Function 通信，不直接访问 `app-crzi248am2gx-api-k93RZBjPykEa-gateway.appmiaoda.com`
