...
| Endpoint | `GET https://app-crzi248am2gx-api-V9gDz8wo0V5L-gateway.appmiaoda.com/getExpressList` |
| Auth 模式 | `platform_managed`（密钥由平台注入） |
| Auth Header | `X-Gateway-Authorization: Bearer ${apiKey}` |
| Content-Type | `application/json` |
| third_part_domain | `app-crzi248am2gx-api-V9gDz8wo0V5L-gateway.appmiaoda.com` |
...
async function getExpressList(type?: string): Promise<Record<string, string>> {
  const url = new URL(
    "https://app-crzi248am2gx-api-V9gDz8wo0V5L-gateway.appmiaoda.com/getExpressList"
  );
  if (type) {
