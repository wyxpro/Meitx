...
| Endpoint | `POST https://app-crzi248am2gx-api-baBwZEjbeNP9-gateway.appmiaoda.com/stock/hs/tick` |
| Auth | `platform_managed`（`X-Gateway-Authorization: Bearer ${INTEGRATIONS_API_KEY}`） |
| Content-Type | `application/x-www-form-urlencoded` |
| Third-party domain | `app-crzi248am2gx-api-baBwZEjbeNP9-gateway.appmiaoda.com` |
...
async function fetchTick(code: string): Promise<TickData> {
  const response = await fetch(
    "https://app-crzi248am2gx-api-baBwZEjbeNP9-gateway.appmiaoda.com/stock/hs/tick",
    {
      method: "POST",
      headers: {
        "Content-Type": "application/x-www-form-urlencoded",
