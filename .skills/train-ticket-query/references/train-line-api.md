...
| Endpoint | `POST https://app-crzi248am2gx-api-DLEO73l7Vjwa-gateway.appmiaoda.com/train/line` |
| 认证方式 | platform_managed（密钥由平台注入） |
| Auth Header | `X-Gateway-Authorization: Bearer ${apiKey}` |
| Content-Type | `application/json;charset=UTF-8` |
| Third-party Domain | `app-crzi248am2gx-api-DLEO73l7Vjwa-gateway.appmiaoda.com` |
...
async function queryTrainLine(trainno: string, date?: string): Promise<TrainLineResult> {
  ...
  const response = await fetch(
    `https://app-crzi248am2gx-api-DLEO73l7Vjwa-gateway.appmiaoda.com/train/line?${params}`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json;charset=UTF-8",
